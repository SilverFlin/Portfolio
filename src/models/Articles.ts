...
const articleMap: Map<number, ArticleViewProps> = new Map();
...
function getArticleById(id: number): ArticleViewProps | undefined {
    return articleMap.get(id);
}

function getArticlesCards(): ArticleCardProps[] {
    const articleCardList: ArticleCardProps[] = [];
    articleMap.forEach((article) => {
        articleCardList.push({
            ...
            date: article.date,
            imageName: article.coverImageURL,
            tags: article.tags
        });
    });
